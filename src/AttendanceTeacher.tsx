...
export interface AttendanceTeacherProps {
  teacherId: number;
}
export const AttendanceTeacher = ({ teacherId }: AttendanceTeacherProps) => {
  const uidState = getTemp<string>("uid");
  const attendances = useIdleStatus(async () => {
    const attendances = await apis.getTeacherAttendance(teacherId);
    return attendances;
  }, []);
  useEffect(() => {
    if (uidState) {
